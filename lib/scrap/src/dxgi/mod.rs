use self::ffi::*;
use std::{io, mem::MaybeUninit, ptr, slice};
use winapi::{
...
impl Capturer {
    pub fn new(display: &Display) -> io::Result<Capturer> {
        ...
        let mut desc = unsafe { MaybeUninit::uninit().assume_init() };
    ...
    unsafe fn ohgodwhat(&mut self, frame: *mut IDXGIResource) -> io::Result<*mut IDXGISurface> {
        ...
        let mut texture_desc = MaybeUninit::uninit().assume_init();
...
impl Displays {
    ...
    fn read_and_invalidate(&mut self) -> Option<Option<Display>> {
        ...
        let desc = unsafe {
            let mut desc = MaybeUninit::uninit().assume_init();
        ...
        if (inner as *const IDXGIOutput1).is_null() {
            unsafe {
                (*self.adapter).Release();
                self.adapter = ptr::null_mut();
            }
